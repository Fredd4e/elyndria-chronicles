#!/usr/bin/env python3
"""
Elyndria Chronicles - v1.5 FINAL (Slider fix + Bigger UI + Cursor hide on RMB)
"""
...
# Options Menu (bigger + better slider)
...
def input(key):
    ...
    if key == 'left mouse down':
        if hasattr(player, 'sword'):
            ...
            if random.random() > 0.6:
                Entity(model='sphere', color=color.orange, scale=0.3, position=player.position + player.forward * 2.5 + (0, 1.2, 0), lifetime=0.25).animate_scale(0.05, duration=0.25)
    if key == 'q':
        if player_stats.mana > 12:
            ...
            for i in range(3):
                invoke(lambda p=start_pos + player.forward * (i*4): Entity(model='sphere', color=color.yellow, scale=0.12, position=p, lifetime=0.4).fade_out(duration=0.4), delay=i*0.08)
            print(f"[COMBAT] Fireball cast! Mana: {player_stats.mana}")
    if key == 'space':
        if on_ground:
            vy = JUMP_FORCE
            on_ground = False
            Entity(model='sphere', color=color.rgb(0.6, 0.5, 0.3), scale=0.8, position=player.position + (0, 0.1, 0), lifetime=0.3).animate_scale(0.1, duration=0.3)
...
# ==================== UPDATE ====================
def update():
    global yaw, pitch, camera_distance, target_yaw, target_pitch, target_distance, velocity, vy, on_ground
    ...
    # WoW-style cursor hiding
    mouse.visible = not held_keys['right mouse']
    
    # Reliable slider sync
    ...
    if move_dir.length() > 0.01:
        ...
        if velocity.length() > MOVE_SPEED: velocity = velocity.normalized() * MOVE_SPEED
    ...
    if abs(player.x) > 125 or abs(player.z) > 125:
        player.position = player.position * 0.92
    mana_text.text = f"Mana: {player_stats.mana}/100"

print("=" * 60)
print("ELY NDRIA CHRONICLES - v1.5 FINAL")
print("Camera Sensitivity slider now works reliably + bigger UI + cursor hides on RMB")
print("Press Esc for Options. Enjoy!")
print("=" * 60)
